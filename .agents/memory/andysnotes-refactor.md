...
**File System Access API + Replit preview iframe:** `showDirectoryPicker()` throws a SecurityError inside the Replit preview pane (cross-origin iframe) — browsers block the picker there. It only works when the app is opened in its own top-level tab (or deployed + opened directly). **How to apply:** feature-detect `window.showDirectoryPicker`, and on failure check `window.self !== window.top` to tell the user to open in a new tab rather than showing a generic error. Local-folder entry is the sidebar `+` button (`handleSidebarAdd` → first press with no workspace calls `connectLocalFolder`), NOT a separate topbar button.
